...
项目地址：[github](https://github.com/KissSheep666/langchain4j-chatAi.git)
...
4、知识库，可上传文件、url，支持的文件格式见[Tika](https://tika.apache.org/2.9.1/formats.html)
  - 同步上传：`POST /document/load/file` / `POST /document/load/url`
  - **异步上传（新增）：** `POST /document/load/file/async` / `POST /document/load/url/async` — 基于 RabbitMQ 消息队列异步处理，避免大文件阻塞请求线程

5、EmbeddingStoreRAG

6、联网搜索websearch，支持开关

7、**RabbitMQ 异步文档处理** — 文档上传后通过消息队列异步解析→向量化→存储，支持高并发场景

8、**单元测试** — 覆盖 ChatController、SwitchQueryRouter、ContentConvert 等核心组件
...
运行单元测试（无需外部依赖，11 个测试可在 CI 环境直接通过）：
...
测试覆盖：
- `ChatControllerTest` — 会话创建、流式对话代理
- `SwitchQueryRouterTest` — 联网开关路由逻辑
- `ContentConvertTest` — RAG 检索结果转换

> `Langchain4jSpringbootApplicationTests` 需要真实 API key，默认禁用。本地运行时取消 `@Disabled` 并配置环境变量 `API_KEY_DASH_SCOPE`。
